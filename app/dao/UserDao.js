...
var insert = function(user, callback) {
    console.log("user:"+user, user.usetype);
...
var modify = function(user, callback) {
    var sql = "UPDATE t_admin SET Admin_name = '" + user.username + "',Admin_password = '" + user.password + "',Admin_type = '" + user.usertype + "' WHERE Admin_id =" + user.id;
    console.log(sql);
    try {
        //执行插入语句，成功返回success
