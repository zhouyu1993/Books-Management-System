...
var jsCookie = window.Cookies

var type = '3'

if (jsCookie) {
  type = jsCookie.get('type')
}
...
adminApp.controller('navCtrl', ['$scope', '$http', '$location',
    function($scope, $http, $location) {
        ...
        var navGroups = [{
            navName: '查看全部书籍', //显示的名称
            urlName: 'borrowingBook' //对应URL的名称
        }];
...
//查看全部书籍
adminApp.controller('borrowingBook', ['$scope', '$http',
    function($scope, $http) {
        $scope.filterOptions = {
            filterText: "",
            useExternalFilter: true
        ...
        $scope.gridOptions = {
            ...
            columnDefs: [{
                ...
                width: '8%',
                // width: 120,
                cellTemplate: '<div><a class="btn btn-xs btn-success feng-btn-modify" ng-click="borrowingBook(row)"  data="{{row.getProperty(col.field)}}">借阅</a></div>'
            }],
            enablePaging: true,
            showFooter: true,
            showGroupPanel: true, //顶部的分组选项，可以拖拽列
        ...
        $scope.borrowingBook = function (row) {
          var currentNum = row.entity.Current_num
...
//添加管理员
adminApp.controller('addAdmin', ['$scope', '$http', function ($scope, $http) {
  //检验通过
  $scope.addAdmin = function(valid, event) {
    ...
    if (['1', '2', '3'].indexOf($scope.uType) < 0) {
      alert("1系统管理员，2图书管理员，3读者");
    ...
    if (valid) {
      ...
      $http({
          ...
          if (response.status === 200) {
              for (var p in $scope.formData) {
                  $scope.uName = $scope.psw = $scope.pswAgain = $scope.uType = '';
              }
              alert(response.data.success);
          }
      }, function errorCallback(response) {
        ...
        $scope.manageAccount = function(valid, event) {
            ...
            if (valid) {
                ...
                $http({
                    ...
                    if (response.status === 200) {
                        for (var p in $scope.formData) {
                            $scope.uName = $scope.psw = $scope.pswAgain = $scope.pswOld = $scope.uType = '';
                        }
                        alert(response.data.success);
                    }
                }, function errorCallback(response) {
